...
class Store {
  ...
  addGroup(name) {
    // Enforce max 2 groups
    if (this.data.groups.length >= 2) return null
    const group = {
      id: 'grp_' + Date.now(),
      name,
      accounts: []
  ...
  addAccount(groupId, name) {
    ...
    if (!group || group.accounts.length >= 2) return null
  ...
  generateAccounts(groupId, count = 2) {
    const group = this.getGroup(groupId)
    if (!group) return
    const existing = group.accounts.length
    const toCreate = Math.min(count, 2 - existing)
